...
fn extract_function_name(fn_def: &str) -> Option<&str> {
  ...
  let function_name_re = Regex::new(r#"^function\s+([^\(\)\{\}\s]+)\s*\("#).unwrap();
...
impl ResourceStorage {
  ...
  pub fn from_resources(resources: impl IntoIterator<Item = Resource>) -> Self {
    ...
    resources.into_iter().for_each(|resource| {
      ...
      #[cfg(not(test))]
      self_.add_resource(resource).unwrap();
    });
  ...
  pub fn add_resource(&mut self, resource: Resource) -> Result<(), AddResourceError> {
    ...
    for ident in core::iter::once(&resource.name).chain(resource.aliases.iter()) {
...
impl From<alloc::string::FromUtf8Error> for AddResourceError {
...
impl From<alloc::string::FromUtf8Error> for ScriptletResourceError {
  fn from(_: alloc::string::FromUtf8Error) -> Self {
    Self::CorruptScriptletContent
  }
}
...
mod scriptlet_storage_tests {
  ...
  fn parse_argslist_quoted() {
    ...
    let args =
      parse_scriptlet_args(r#"debug-scriptlet, test\,test, test\test, "test\test", 'test\test', "#)
        .unwrap();
    assert_eq!(
      args,
      vec![
  ...
  fn get_patched_scriptlets() {
    ...
    assert_eq!(
      resources
        .get_scriptlet_resources([("alert, Uh oh\\, check the logs...", Default::default())]),
    ...
    assert_eq!(
      resources
        .get_scriptlet_resources([("set-local-storage-item, Test, $remove$", Default::default())]),
  ...
  fn parse_template_file_format() {
    ...
    assert_eq!(
      resources
        .get_scriptlet_resources([("abort-on-property-read, write tests", Default::default())]),
  ...
  fn patch_argslist_many_args() {
    ...
    let args = parse_scriptlet_args(
      "acs, this, probably, is, going, to, break, brave, and, crash, it, instead, of, ignoring, it",
    )
    .unwrap();
    assert_eq!(
      args,
      vec![
        "acs", "this", "probably", "is", "going", "to", "break", "brave", "and", "crash", "it",
