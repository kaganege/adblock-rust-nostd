...
impl NetworkFilter {
    ...
    if let Some(options) = parsed.options {
      ...
      options.into_iter().for_each(|option| {
        match option {
          ...
          NetworkFilterOption::ThirdParty(false) | NetworkFilterOption::FirstParty(true) => {
            mask.set(NetworkFilterMask::THIRD_PARTY, false)
          }
          NetworkFilterOption::ThirdParty(true) | NetworkFilterOption::FirstParty(false) => {
  ...
  pub fn parse_hosts_style(hostname: &str, debug: bool) -> Result<Self, NetworkFilterError> {
    ...
    if normalized_host.is_ascii() {
      ...
      hostname.push_str(
        &idna::domain_to_ascii(normalized_host).map_err(|_| NetworkFilterError::PunycodeError)?,
      );
    }
    hostname.push('^');
    ...
    match &self.filter {
      FilterPart::Simple(f) => {
        if !self.is_complete_regex() {
          let skip_last_token = (self.is_plain() || self.is_regex()) && !self.is_right_anchor();
    ...
    .map(|hostname| {
      if is_anchored_by_hostname(
        ...
        match &filter.filter {
          ...
          FilterPart::Simple(f) => get_url_after_hostname(&request_url, hostname).contains(f),
          FilterPart::AnyOf(filters) => {
            let url_after_hostname = get_url_after_hostname(&request_url, hostname);
            for f in filters {
  ...
  if let Some(excluded_domains) = filter.opt_not_domains.as_ref() {
    if let Some(source_hashes) = request.source_hostname_hashes.as_ref() {
      ...
      if let Some(excluded_domains_union) = filter.opt_not_domains_union {
        ...
        if source_hashes
          .iter()
          .any(|h| (h & excluded_domains_union == *h) && utils::bin_lookup(excluded_domains, *h))
        {
          return false;
        }
...
mod parse_tests {
  ...
  fn parses_hostname_anchor_pattern() {
    ...
    {
      let filter =
        NetworkFilter::parse("||foo.com/bar/baz$important", true, Default::default()).unwrap();
      let mut defaults = default_network_filter_breakdown();
      defaults.hostname = Some(String::from("foo.com"));
  ...
  fn parses_csp() {
    ...
    }
    {
      // parses simple CSP
      let filter =
        NetworkFilter::parse(r#"||foo.com$csp=self bar """#, true, Default::default()).unwrap();
  ...
  fn parses_domain() {
    ...
    {
      let filter =
        NetworkFilter::parse("||foo.com$domain=bar.com|baz.com", true, Default::default()).unwrap();
    ...
    {
      let filter =
        NetworkFilter::parse("||foo.com$domain=foo|~bar|baz", true, Default::default()).unwrap();
  ...
  fn parses_redirects() {
    ...
    {
      let filter =
        NetworkFilter::parse("||foo.com$redirect=bar.js", true, Default::default()).unwrap();
  ...
  fn parses_removeparam() {
    ...
    {
      let filter =
        NetworkFilter::parse("||foo.com^$removeparam=test", true, Default::default()).unwrap();
  ...
  fn parses_match_case() {
    ...
    {
      assert!(
        NetworkFilter::parse("||foo.com$image,match-case", true, Default::default()).is_err()
      );
    }
    {
      assert!(
        NetworkFilter::parse("||foo.com$media,match-case,image", true, Default::default()).is_err()
      );
    }
  ...
  fn handles_content_type_options() {
    ...
    for option in options {
      ...
      {
        let filter =
          NetworkFilter::parse(&format!("||foo.com${}", option), true, Default::default()).unwrap();
        let mut defaults = default_network_filter_breakdown();
        defaults.hostname = Some(String::from("foo.com"));
        defaults.is_hostname_anchor = true;
        defaults.is_plain = true;
  ...
  fn binary_serialization_works() {
    ...
    }
    {
      ...
      assert_eq!(defaults, NetworkFilterBreakdown::from(&decoded));
      assert!(RegexManager::default().matches(&decoded, "bar/"));
    }
  }
...
mod match_tests {
  ...
  fn check_pattern_exception_works() {
    {
      ...
      let network_filter = NetworkFilter::parse(filter, true, Default::default()).unwrap();
      let request =
        request::Request::new(url, "https://www.gamespot.com/metro-exodus/", "script").unwrap();
      assert!(
  ...
  fn check_ws_vs_http_matching() {
    ...
    assert!(network_filter.matches_test(
      &request::Request::new("wss://example.com", "https://4shared.com", "websocket").unwrap()
    ));
    assert!(!network_filter.matches_test(
    ...
    assert!(!network_filter.matches_test(
      &request::Request::new("http://example.com", "https://4shared.com", "websocket").unwrap()
    ));
    assert!(!network_filter.matches_test(
      &request::Request::new("https://example.com", "https://4shared.com", "websocket").unwrap()
    ));
  }
  ...
  fn check_options_works() {
    ...
    // ~third-party
    {
      ...
      let request = request::Request::new("https://foo.com/bar", "http://baz.foo.com", "").unwrap();
      assert!(check_options(&network_filter, &request));
    }
    {
      let network_filter =
        NetworkFilter::parse("||foo$~third-party", true, Default::default()).unwrap();
      let request = request::Request::new("https://foo.com/bar", "http://baz.bar.com", "").unwrap();
      assert!(!check_options(&network_filter, &request));
    }
    ...
    // opt-domain
    {
      ...
      let request = request::Request::new("https://foo.com/bar", "http://foo.com", "").unwrap();
      assert!(check_options(&network_filter, &request));
    }
    {
      let network_filter =
        NetworkFilter::parse("||foo$domain=foo.com", true, Default::default()).unwrap();
      let request = request::Request::new("https://foo.com/bar", "http://bar.com", "").unwrap();
      assert!(!check_options(&network_filter, &request));
    }
  ...
  fn check_domain_option_subsetting_works() {
    {
      ...
      assert!(!network_filter.matches_test(
        &request::Request::new("http://example.net/adv", "http://foo.example.com", "").unwrap()
      ));
      assert!(!network_filter.matches_test(
        &request::Request::new(
      ...
      assert!(network_filter.matches_test(
        &request::Request::new("http://example.net/adv", "http://bar.example.com", "").unwrap()
      ));
      assert!(!network_filter.matches_test(
        &request::Request::new("http://example.net/adv", "http://anotherexample.com", "").unwrap()
      ));
    }
    {
      let network_filter = NetworkFilter::parse(
    ...
    }
    {
      ...
      assert!(network_filter
        .matches_test(&request::Request::new("http://example.net/adv", "http://com", "").unwrap()));
      assert!(!network_filter.matches_test(
        &request::Request::new("http://example.net/adv", "http://foo.com", "").unwrap()
      ));
      assert!(!network_filter.matches_test(
        &request::Request::new("http://example.net/adv", "http://subfoo.foo.com", "").unwrap()
      ));
      assert!(network_filter.matches_test(
