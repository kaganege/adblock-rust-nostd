...
impl CosmeticFilter {
  ...
  fn parse_before_sharp(
    ...
    #[inline]
    fn sorted_or_none<T: core::cmp::Ord>(mut vec: Vec<T>) -> Option<Vec<T>> {
      if !vec.is_empty() {
        vec.sort();
        Some(vec)
      } else {
...
mod css_validation {
  ...
  fn to_css_nested<'i>(
    ...
    while let Ok(arg) = arguments.next_including_whitespace() {
      ...
      if let Some(closing_token) = nested_matching_close(arg) {
        ...
        closing_token
          .to_css(&mut inner)
          .map_err(|_| arguments.new_custom_error(SelectorParseErrorKind::InvalidState))?;
      }
    }
    Ok(inner)
  }
  ...
  fn bad_selector_inputs() {
    ...
    assert!(
      validate_css_selector(r#"javascript:alert("All pseudo-classes are valid")"#, false).is_ok()
    );
  ...
  fn styles() {
    ...
    check_parse_result(
      ...
      CosmeticFilterBreakdown {
        ...
        action: Some(CosmeticFilterAction::Style(
          "background-color: #e3e2dd !important; background-image: none !important;".into(),
        )),
        ..Default::default()
      },
    );
...
mod matching_tests {
  ...
  fn multiple_selectors() {
    ...
    assert!(parse_cf(
      "googledrivelinks.com###wpsafe-generate, #wpsafe-link:style(display: block !important;)"
    )
    .is_ok());
  }
