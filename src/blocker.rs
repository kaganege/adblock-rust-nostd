...
impl Blocker {
  ...
  pub fn check_parameterised(
    ...
    let redirect: Option<String> = redirect_resource.and_then(|resource_name| {
      resources.get_redirect_resource(resource_name).or({
        ...
        #[cfg(test)]
        eprintln!(
          "Matched rule with redirect option but did not find corresponding resource to send"
        );
        None
      })
    });
  ...
  fn apply_removeparam(
    ...
    impl<'a> core::fmt::Display for QParam<'a> {
...
/// sorted order.
fn insert_dup<K, V, H: core::hash::BuildHasher>(map: &mut HashMap<K, Vec<V>, H>, k: K, v: V)
where
  ...
  V: PartialOrd,
{
  ...
  match entry.binary_search_by(|f| f.partial_cmp(&v).unwrap_or(core::cmp::Ordering::Equal)) {
    Ok(_pos) => (), // Can occur if the exact same rule is inserted twice. No reason to add anything.
    Err(slot) => entry.insert(slot, v),
  }
}

fn vec_hashmap_len<K: core::cmp::Eq + core::hash::Hash, V, H: core::hash::BuildHasher>(
...
mod blocker_tests {
  ...
  fn redirect_exception() {
    ...
    assert_eq!(
      matched_rule.exception,
      Some("@@||imdb-video.media-imdb.com^$domain=imdb.com,redirect=noop-0.1s.mp3".to_string())
    );
  }
  ...
  fn get_csp_directives() {
    ...
    }
    {
      ...
      assert_eq!(
        blocker.get_csp_directives(
          &Request::new("https://example.com", "https://vidoza.net", "subdocument").unwrap()
        ),
        Some(String::from("worker-src 'none'"))
      );
    }
    ...
    }
    {
      ...
      assert_eq!(
        blocker.get_csp_directives(
          &Request::new("https://1337x.to/no-csp", "https://1337x.to", "subdocument").unwrap()
        ),
        None
      );
    }
  ...
  fn tags_enable_works() {
    ...
    assert_eq!(
      ...
      hashbrown::HashSet::from_iter([String::from("stuff")].into_iter())
    );
  ...
  fn tags_enable_adds_tags() {
    ...
    assert_eq!(
      blocker.tags_enabled,
      hashbrown::HashSet::from_iter([String::from("brian"), String::from("stuff")].into_iter())
    );
  ...
  fn tags_disable_works() {
    ...
    assert_eq!(
      blocker.tags_enabled,
      hashbrown::HashSet::from_iter([String::from("brian")].into_iter())
    );
    assert_eq!(vec_hashmap_len(&blocker.filters_tagged.filter_map), 2);
  ...
  fn filter_add_tagged() {
    ...
    blocker
      ...
      .unwrap();
    blocker
      .add_filter(NetworkFilter::parse("||brave.com$tag=brian", true, Default::default()).unwrap())
      .unwrap();
  ...
  fn generichide() {
    ...
    blocker
      .add_filter(
        NetworkFilter::parse("@@||example.com$generichide", true, Default::default()).unwrap(),
      )
      .unwrap();
